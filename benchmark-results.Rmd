...
relative <- d %>%
  ...
  group_by(benchmark, raptorjit) %>% mutate(relativemean = overallmean / mean(cycles))

ggplot(aes(y=relativemean, x=benchmark, fill=raptorjit), data=relative) +
...
ggplot(aes(y=rsd, x=benchmark, fill=raptorjit), data=rsd) +
  geom_bar(stat = "identity", position = "dodge") +
...
ggplot(aes(y=ipc, x=benchmark, fill=raptorjit), data=ipc) +
  geom_bar(stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90)) +
